...
{% step %}
`CitaController` recibe y delega a `CitaServiceImpl.guardar`.
{% endstep %}

{% step %}
El servicio normaliza fecha.
{% endstep %}

{% step %}
Valida conflictos de horario para médico y paciente.
{% endstep %}

{% step %}
Consulta remoto a `msvc-medico` y `msvc-paciente` para confirmar estado `ACTIVO`.
{% endstep %}

{% step %}
Si todo es válido, guarda en BD y responde `201`.
{% endstep %}
{% endstepper %}
...
{% step %}
`CitaServiceImpl.porIdConDetalle` obtiene cita local.
{% endstep %}

{% step %}
Enriquecimiento remoto:
...
{% step %}
Devuelve un DTO agregado con toda la información disponible.
{% endstep %}
{% endstepper %}
...
{% step %}
`RdfGraphServiceImpl` pide el grafo clínico a `SemanticGraphServiceImpl`.
{% endstep %}

{% step %}
`SemanticGraphServiceImpl` consume `msvc-cita` con detalle agregado.
{% endstep %}

{% step %}
Se mapea a DTO semántico.
{% endstep %}

{% step %}
Se convierte a modelo RDF (Jena).
{% endstep %}
...
{% step %}
Se serializa en Turtle/RDFXML/JSON-LD.
{% endstep %}
{% endstepper %}
...
{% stepper %}
{% step %}
El cliente envía pregunta a `POST /semantic/nl/query`.
{% endstep %}
...
{% step %}
Ejecuta SPARQL sobre modelo completo vía `SparqlQueryServiceImpl`.
{% endstep %}

{% step %}
Devuelve respuesta con pregunta, SPARQL generado y resultados.
{% endstep %}
{% endstepper %}
